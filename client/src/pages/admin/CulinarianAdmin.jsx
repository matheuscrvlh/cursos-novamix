...
export default function CulinarianAdmin() {
    ...
    async function editarCulinarian() {
        try {
            ...
            if (culinarianEditar.foto instanceof File) {
                formData.append('foto', culinarianEditar.foto)
            };

            await editCulinarian(formData);

            setCulinarianEditar(CULINARIAN_VAZIO);
            setStep('close')
    ...
    function handleViewCulinarian(culinarianId) {
        setStep('viewCulinarian');

        const culinaristaFiltrada = culinaristas.find(c => c.id === culinarianId);
                ...
                <div className='mb-6'>
                    ...
                    <div className='flex gap-3 mt-2'>
                        ...
                        <Button
                            ...
                            onClick={() => {
                                if (!culinarianEditar.cursoAtual) return
                                setCulinarianEditar(prev => ({
                                    ...prev,
                                    cursos: [...prev.cursos, prev.cursoAtual],
                                    cursoAtual: ''
                                }))
                            }}
                        >
